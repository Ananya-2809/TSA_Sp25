...
#decompose (can choose additive or multiplicative format) or STL (seasonal-trend-linear?)
#uses the moving average method for calculating the trend
...
#The NA's come from the trend component as it is using a centered moving average (so, we lose the first and last 6 months).. thus, the random component has the NA's too
...
#boxplot for each month even after removing the seasonal component
ggplot(df_inflow_random, aes(x=month, y=inflow)) +
            geom_boxplot()

#lines for the interquartile range
...
print(grubbs_test) #look at the p-value to find the decision - test the statistical significance
...
#all the values outside the red lines in the above plot were replaced using tsclean
#this just looks at the +/- 3 std.dev. and not statistical significance of the outliers
