...
library(lubridate) #parse the column that has the time - convert to date
library(ggplot2)
library(forecast)
library(Kendall)
library(tseries)
...
```{r}
#Remove last year by replacing current data frame (removing 2011 as a whole as we're looking at a seasonal dataset with a frequency of 12 - to keep it consistent)
...
#lm() from package 'stats', where lm = linear model
linear_trend_model = lm(inflow_data[,iHP+1] ~ t) #1 is for the date column
...
#using the linear model as it came out to be significant. otherwise, we would go for the moving average model
...
#seasonaldummy fn. and run a simple regression on the dummies using lm
#only works if Y is a time series and if frequency is specified

dummies <- seasonaldummy(ts_detrend)
#default of fn. creates only till Jan-Nov, it takes the Dec as the reference point (intercept point)

seas_means_model = lm(detrend_inflow ~ dummies)
summary(seas_means_model)
#overall the model is significant, but a lot of the dummies are not..
...
```{r}
#specify which hydropower timeseries to analyse
iHP <- 6
...
summary(SeasonalMannKendall(ts_inflow_data))

library(trend)
#gives summary season by season.. doesn't dilute the results
#important for climate data where we see wet seasons getting wetter and dry seasons getting drier
#summary(smk.test(ts_inflow_data))
...
print(cor(inflow_data_yearly$inflow_data, my_year, method = "spearman"))

#with cor.test you can get test statistics
print(cor.test(inflow_data_yearly$inflow_data, my_year, method = "spearman"))

#try removing the seasonality and run the spearman test
...
Run the stationary tests for the other variables HP2, HP3, ...
